...
def display_files_recursively(folder_path:str, indent:str='', 
                              file_suffixes:list=[".py", ".cpp", ".cs", ".md", ".txt"]) -> str:
    """Recursively lists files with specific suffixes from a given directory and its subdirectories.

    This function searches through the directory structure starting from `folder_path` and 
    returns a string representation of the directory hierarchy containing only the files 
    that match any of the provided `file_suffixes`. Each level in the hierarchy increases 
    the indentation in the returned string, enhancing readability.

    Args:
    - folder_path (str): Path to the starting folder from which the search begins.
    - indent (str, optional): The indentation string for the current level of the directory hierarchy. 
                              Defaults to an empty string. Typically used internally for recursive calls.
    - file_suffixes (list of str, optional): A list of file suffixes (extensions) to consider 
                                             while listing files. Defaults to [".py", ".cpp", ".cs", ".md", ".txt"].

    Returns:
    - str: A string representation of the directory hierarchy with files matching the provided suffixes. 
           Directories are only displayed if they contain at least one valid file or subdirectory 
           with valid files.

    Note:
    - The function assumes that the provided `folder_path` exists and is accessible.
    """
    ret = ""
...
def find_all_substr(string, substr):
    ...
    This function scans the input string for all instances of the specified substring and 
    returns a list of indices where these instances start. If the substring is not found 
    ...
    Returns:
    - list of int: A list of starting indices where the substring is found. The list is 
                   empty if the substring is not found.
    """
    start_index = 0
    positions = []
...
def get_exp_id(result_path: str) -> str:
    ...
    This function scans the directory specified by `ckpt_path` to identify existing 
    experiment directories. The experiment ID is derived from the first three characters 
    of directory names, which are expected to be integers. The function then increments 
    the highest found experiment ID by one to generate the next ID. If no existing 
    experiment directories are found, the function returns an ID of "000".

    Args:
    - ckpt_path (str): Path to the directory containing experiment directories.

    Returns:
    - str: The next experiment ID, zero-padded to ensure a length of three characters.

    Note:
    Any directories that do not conform to the expected naming convention (i.e., directories
    without an integer as the first three characters) are ignored.
    """
...
def get_spm_dataset(phase: str, mode: str, with_self_instruct: bool = False) -> Dataset:
    """Retrieves the specified dataset based on the provided phase and mode, with an optional inclusion of self-instructed data.

    This function loads datasets based on the given phase (e.g., 'baseline', 'pretrain', 'finetune') and mode 
    (e.g., 'train', 'test'). Depending on the provided phase, different data paths are chosen, and if 
    `with_self_instruct` is set to True, the self-instructed data path is also considered. However, 
    self-instructed data is not used in the 'finetune' phase.

    Args:
    - phase (str): The phase of the data. Expected values are 'baseline', 'pretrain', or 'finetune'.
    - mode (str): The mode of the data. Expected values are 'train' or 'test'.
    - with_self_instruct (bool, optional): Whether to include self-instructed data. Defaults to False.

    Returns:
    - Dataset: The specified dataset loaded from the chosen data files, shuffled with a seed of 42.

    Raises:
    - ValueError: If an invalid mode or phase is provided.

    Note and TODO:
    The actual paths like 'pretrain_raw_data_path', 'self_instruct_data_path', etc., are assumed to be available 
    in the current scope. Make it more flexible and formal.
    """
...
def save_data(data: dict, train_path: str, test_path: str, train_percent: float=0.7) -> None:
    """
